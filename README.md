# python-practice
Creating python-virtual environments to avoid version conflicts.
Instructions on how to clone this repo to your local.
open your CMD
git clone https://github.com/Jesh-Koppolu/python-virtual-environment.git
cd python-virtual-environment
python -m venv env
.\env\scripts\activate
pip install -r requirements.txt  /*This command will help you install all the packages with same versions in your virtual environment*/
